...
class RedTeam extends Team {
  public static final int INFORM_ABOUT_BASE = 5;
  PVector base1, base2;
...
class RedBase extends Base implements RedRobot {
  ...
  void go() {
    ...
    if ((brain[5].x > 0) && (energy >= 1000 + harvesterCost)) {
      ...
      if ((int)random(3) == 0)
        // creates a new harvester with 33% chance
        brain[5].y++;
      else if ((int)random(2) == 0)
        // creates a new rocket launcher with 33% chance
        brain[5].x++;
      else
        // creates a new explorer with 33% chance
        brain[5].z++;
    }
    ...
    // if friend rocket launcher in sight, send info about base Target
    if(brain[8].x != 0) {
      ArrayList<Robot> listLauncher = perceiveRobots(friend, LAUNCHER);
      if(listLauncher != null) {
        for(int i =0; i < listLauncher.size() ; i++) {
          Robot bobby = listLauncher.get(i);
          informAboutBase( bobby, new PVector(brain[8].x,brain[8].y));
        }         
      }
  ...
  void handleMessages() {
    ...
    for (int i=0; i<messages.size(); i++) {
      ...
      }else if(msg.type == RedTeam.INFORM_ABOUT_BASE){
          brain[8].x=msg.args[0];
          brain[8].y=msg.args[1];
      }
      }
...
//informAboutBase
  void informAboutBase(Base bas, Base target) {
    // check that bob and target both exist and distance less than max range
    if ((bas != null) && (target != null) && (distance(bas) < messageRange)) {
      // build the message...
      ...
      args[2] = target.breed;
      args[3] = target.who;      
      Message msg = new Message(RedTeam.INFORM_ABOUT_BASE, who, bas.who, args);
      // ...and add it to bob's messages queue
      bas. messages.add(msg);
    }
  }
//informAboutBase
  void informAboutBase(Base bas, PVector coorEBase) {
    // check that bob and target both exist and distance less than max range
    if ((bas != null) && (coorEBase != null) && (distance(bas) < messageRange)) {
      // build the message...
      float[] args = new float[4];
      args[0] = coorEBase.x;
      args[1] = coorEBase.y;
...
//informAboutBase
  void informAboutBase(Robot bob, PVector coorEBase) {
    // check that bob and target both exist and distance less than max range
    if ((bob != null) && (coorEBase != null) && (distance(bob) < messageRange)) {
      // build the message...
      float[] args = new float[4];
      args[0] = coorEBase.x;
      args[1] = coorEBase.y;
      Message msg = new Message(RedTeam.INFORM_ABOUT_BASE, who, bob.who, args);
      // ...and add it to bob's messages queue
...
class RedExplorer extends Explorer implements RedRobot {
  ...
  void driveHarvesters() {
    ...
    //if found a burger or has a burger in memory
    if (brain[2].x!=0|| brain[2].y!=0) {
      ...
      if (harvey != null){
        ...
        informAboutFood(harvey, new PVector(brain[2].x,brain[2].y));
      }
    }
  }
  ...
  void driveRocketLaunchers() {
    ...
    if (bob != null) {
      // if one is seen, stock position in brain[0]
      brain[0].x=bob.pos.x;
      brain[0].y=bob.pos.y;
      //then look for a friend rocket launcher
  ...
  void lookForEnnemyBase() {
    ...
    if (babe != null) {
      // if one is seen, stock position in brain[1]
      brain[1].x = babe.pos.x;
      brain[1].y = babe.pos.y;
      brain[1].z = babe.breed;
  ...
  // callAboutEnemyBase
  // =================
  // > give localization of ennemy bases...
  //
  void callAboutEnemyBase() {
    // if localized an ennemy base
    if (brain[1] != null) {
      //look for a friend launcher
      PVector eBase = new PVector(brain[1].x,brain[1].y);
      RocketLauncher launcher = (RocketLauncher)oneOf(perceiveRobots(friend, LAUNCHER));
      if (launcher != null)
        // if one is seen, send a message with the localized ennemy base
        informAboutBase(launcher, eBase);
      // look for a friend base
      Base basy = (Base)oneOf(perceiveRobots(friend, BASE));
      if (basy != null)
        // if one is seen, send a message with the localized ennemy base
        informAboutBase(basy, eBase);
    }
  }
    ...
    if (brain[4].x == 1) {
      ...
      if (target()){
        ...
        if(brain[4].y==1){
          ...
          if((distance(new PVector(brain[1].x,brain[1].y)) < launcherPerception) && (Base)oneOf(perceiveRobots(ennemy, BASE))==null ){
            brain[1].x=0;
            brain[1].y=0;
          }else{
            heading=towards(new PVector(brain[1].x,brain[1].y))+ random(-radians(20), radians(20));
            tryToMoveForward();
          }
  ...
  void handleMessages() {
    ...
    for (int i=0; i<messages.size(); i++) {
      ...
      if (msg.type == INFORM_ABOUT_TARGET) {
        ...
        p.y = msg.args[1];
        if(brain[0]!=null){
          float distanceActual = distance(brain[0]);
          float distanceNew=distance(new PVector(p.x,p.y));
          if (distanceNew < distanceActual-10) {
            // record the position in the brain
            brain[0].x = p.x;
            brain[0].y = p.y;
          }
