...
//informAboutBase
  void informAboutBase(Base bas, Base target) {
    ...
    if ((bas != null) && (target != null) && (distance(bas) < messageRange)) {
      ...
      args[2] = target.breed;
      args[3] = target.who;      
      Message msg = new Message(INFORM_ABOUT_BASE, who, bas.who, args);
      // ...and add it to bob's messages queue
...
//informAboutBase
  void informAboutBase(Robot bob, PVector coorEBase) {
    ...
    if ((bob != null) && (coorEBase != null) && (distance(bob) < messageRange)) {
      ...
      args[0] = coorEBase.x;
      args[1] = coorEBase.y;
      Message msg = new Message(INFORM_ABOUT_BASE, who, bob.who, args);
      // ...and add it to bob's messages queue
      bob. messages.add(msg);
    }
  }*/
...
/*      Message msg = new Message(INFORM_ABOUT_TARGET, who, bob.who, args);
  ...
  // -----
  // > bob = the receiver
  // > target = the id of the target robot
  //
  void informAboutTarget(Robot bob,int id) {
    Robot ene = game.getRobot(id);
    // check that bob and target both exist and distance less than max range
    if ((bob != null) && (ene != null) && (distance(bob) < messageRange)) {
      // build the message...
      float[] args = new float[4];
      args[0] = ene.pos.x;
      args[1] = ene.pos.y;
